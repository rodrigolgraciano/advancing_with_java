== Suggest an agents.md while generating OpenRewrite recipes for upgrading Java create a recipe to upgrade to latest Java features, but do not use the "var". For JUnit 4 -> 5 replace all test method signatures to not have the "test" prefix.
...
* upgrading to the latest Java target via OpenRewrite’s composite migration recipe (currently Java **25**) ([OpenRewrite Docs][1])
* **never** introducing `var` (i.e., don’t activate any `UseVar`/`...var...` recipes) ([OpenRewrite Docs][2])
* migrating **JUnit 4 → JUnit 5** and removing the `test` method-name prefix ([OpenRewrite Docs][3])
...
* `UpgradeToJava25` is OpenRewrite’s current “migrate to Java 25” composite. ([OpenRewrite Docs][1])
* `RemoveTestPrefix` is the purpose-built recipe for dropping `test*` prefixes on JUnit 5 test methods. ([OpenRewrite Docs][4])
* Avoiding `var` is accomplished by **not** activating `UseVar` or any `...lang.var...` recipes. ([OpenRewrite Docs][2])

[1]: https://docs.openrewrite.org/recipes/java/migrate/upgradetojava25 "Migrate to Java 25 | OpenRewrite Docs"
[2]: https://docs.openrewrite.org/recipes/java/migrate/lang/usevar?utm_source=chatgpt.com "Use local variable type inference - Java"
[3]: https://docs.openrewrite.org/recipes/java/testing/junit5/junit4to5migration?utm_source=chatgpt.com "JUnit Jupiter migration from JUnit 4.x"
[4]: https://docs.openrewrite.org/recipes/java/testing/cleanup/removetestprefix "Remove test prefix from JUnit 5 tests | OpenRewrite Docs"
